...
class newDatum:
    ...
    def __init__(self, datumName):
        ...
        self.containers = ['App::Part',
                           'PartDesign::Body', 'App::DocumentObjectGroup']
        if self.datumName == 'Point':
            self.datumType = 'PartDesign::Point'
            self.menutext = App.Qt.translate("Assembly", "New Point")
            self.tooltip = App.Qt.translate("Assembly", "Create a new Datum Point in a Part")
            ...
            self.menutext = App.Qt.translate("Assembly", "New Axis")
            self.tooltip = App.Qt.translate(
                "Assembly", "Create a new Datum Axis in a Part")
            ...
            self.menutext = App.Qt.translate("Assembly", "New Plane")
            self.tooltip = App.Qt.translate(
                "Assembly", "Create a new Datum Plane in a Part")
            ...
            self.menutext = App.Qt.translate(
                "Assembly", "New Coordinate System")
            self.tooltip = App.Qt.translate(
                "Assembly", "Create a new Coordinate System in a Part")
            self.icon = os.path.join(
                Asm4.iconPath, 'Asm4_CoordinateSystem.svg')
            self.datumColor = []
            self.datumAlpha = []
        elif self.datumName == 'Sketch':
            self.datumType = 'Sketcher::SketchObject'
            self.menutext = App.Qt.translate("Assembly", "New Sketch")
            self.tooltip = App.Qt.translate(
                "Assembly", "Create a new Sketch in a Part")
    ...
    def Activated(self):
        ...
        # something went wrong
            ...
            Asm4.warningBox(App.Qt.translate("Assembly", "I can't create a ") +
                            self.datumType+App.Qt.translate("Assembly", " with the current selections"))
        ...
        if parentContainer:
            ...
            text, ok = QtGui.QInputDialog.getText(None, App.Qt.translate("Assembly", 'Create new ')+self.datumName,
                                                  App.Qt.translate("Assembly", 'Enter ')+self.datumName+App.Qt.translate("Assembly", ' name :')+' '*40, text=proposedName)
            if ok and text:
                ...
                Gui.Selection.addSelection(
                    App.ActiveDocument.Name, parentContainer.Name, createdDatum.Name+'.')
...
class newHole:
    def GetResources(self):
        return {"MenuText": App.Qt.translate("Assembly", "New Hole Axis"),
                "ToolTip": App.Qt.translate("Assembly", "Create a Datum Axis attached to a hole"),
    ...
    def Activated(self):
        ...
        for i in range(len(edges)):
            ...
            if parentPart:
                ...
                if parentPart and (parentPart.TypeId == 'App::Part' or parentPart.TypeId == 'PartDesign::Body'):
                    ...
                    while parentDoc.getObject('HoleAxis_'+str(instanceNum)):
                        instanceNum += 1
                    axis = parentPart.newObject(
                        'PartDesign::Line', 'HoleAxis_'+str(instanceNum))
            ...
            #
                ...
                FCC.PrintMessage(App.Qt.translate("Assembly", 'Datum objects can only be created inside Part or Body containers'))
...
createDatumList = ['Asm4_newLCS',
                   ...
                   'Asm4_newPoint',
                   'Asm4_newHole']
Gui.addCommand('Asm4_createDatum', Asm4.dropDownCmd(
    createDatumList, App.Qt.translate("Assembly", 'Create Datum Object')))
