...
def makeAssembly():
    ...
    def GetResources(self):
        tooltip = App.Qt.translate(
            "Assembly", "<p>Create a new Assembly container</p>")
        iconFile = os.path.join(Asm4.iconPath, 'Asm4_Model.svg')
        return {"MenuText": App.Qt.translate("Assembly", "New Assembly"), "ToolTip": tooltip, "Pixmap": iconFile}
    ...
    def Activated(self):
        ...
        if assy is not None:
            if assy.TypeId == 'App::Part':
                message = App.Qt.translate(
                    "Assembly", "This document already contains a valid Assembly, please use it")
                Asm4.warningBox(message)
                # set the Type to Assembly
                assy.Type = 'Assembly'
            else:
                message = App.Qt.translate("Assembly", "This document already contains another FreeCAD object called \"Assembly\", but it's of type \"") + \
                    assy.TypeId + \
                    App.Qt.translate(
                        "Assembly", "\", unsuitable for an assembly. I can\'t proceed.")
                Asm4.warningBox(message)
            # abort
            return
        ...
        if partsGroup.TypeId == 'App::DocumentObjectGroup':
            ...
            Asm4.warningBox(
                App.Qt.translate("Assembly", 'There seems to already be a Parts object, you might get unexpected behaviour'))
