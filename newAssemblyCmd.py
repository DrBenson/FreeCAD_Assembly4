...
def makeAssembly():
    ...
    def GetResources(self):
        tooltip = _atr("Asm4_newAssembly",
                       "<p>Create a new Assembly container</p>")
        iconFile = os.path.join(Asm4.iconPath, 'Asm4_Model.svg')
        return {"MenuText": _atr("Asm4_newAssembly", "New Assembly"), "ToolTip": tooltip, "Pixmap": iconFile}
    ...
    def Activated(self):
        ...
        if assy is not None:
            if assy.TypeId == 'App::Part':
                message = _atr(
                    "Asm4_newAssembly", "This document already contains a valid Assembly, please use it")
                Asm4.warningBox(message)
                # set the Type to Assembly
                assy.Type = 'Assembly'
            else:
                message = _atr("Asm4_newAssembly", "This document already contains another FreeCAD object called \"Assembly\", "
        ...
        if partsGroup.TypeId == 'App::DocumentObjectGroup':
            ...
            Asm4.warningBox(_atr(
                "Asm4_newAssembly", 'There seems to already be a Parts object, you might get unexpected behaviour'))
