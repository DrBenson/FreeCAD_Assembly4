...
class placeLinkUI():
    ...
    def __init__(self):
        ...
        if Asm4.isAsm4EE(self.selectedObj):
            ...
            FCC.PrintWarning(App.Qt.translate(
                "Assembly", "WARNING : unsupported Assembly/Solver/Part combination, you shouldn't be seeing this\n"))
        ...
        lcs_found = self.partLCSlist.findItems(
            old_linkLCS, QtCore.Qt.MatchExactly)
        # ... or with (Name)
        if not lcs_found:
            lcs_found = self.partLCSlist.findItems(
        ...
        lcs_found = self.attLCSlist.findItems(
            old_attLCS, QtCore.Qt.MatchExactly)
    ...
    def accept(self):
        if self.Apply():
            ...
            FCC.PrintWarning(App.Qt.translate(
                "Assembly", "Problem in selections\n"))
            return
    ...
    def clicked(self, button):
        if button == QtGui.QDialogButtonBox.Apply:
            ...
            if self.selectedObj.ExpressionEngine:
                ...
                confirmed = Asm4.confirmBox(App.Qt.translate("Assembly", 'This command will release all attachments on ') +
                                            msgName+App.Qt.translate("Assembly", ' and set it to manual positioning in its current location.'))
            ...
            if confirmed:
                ...
                FCC.PrintMessage(App.Qt.translate(
                    "Assembly", "Part is now manually placed\n"))
                self.finish()
            else:
                FCC.PrintMessage(App.Qt.translate(
                    "Assembly", "Part untouched\n"))
                self.finish()
    ...
    def onLCSclicked(self):
        ...
        if sel:
            Gui.Selection.addSelection(
                self.activeDoc.Name, self.rootAssembly.Name, sel)
    ...
    def reorientLink(self):
        moveXYZ = App.Placement(App.Vector(
            self.Xtranslation, self.Ytranslation, self.Ztranslation), self.old_LinkRotation)
        ...
        rotationX = App.Placement(App.Vector(0.00, 0.00, 0.00), App.Rotation(
            App.Vector(1, 0, 0), self.XrotationAngle - self.old_LinkRotation.toEuler()[0]))
        rotationY = App.Placement(App.Vector(0.00, 0.00, 0.00), App.Rotation(
            App.Vector(0, 1, 0), self.YrotationAngle - self.old_LinkRotation.toEuler()[1]))
        rotationZ = App.Placement(App.Vector(0.00, 0.00, 0.00), App.Rotation(
            App.Vector(0, 0, 1), self.ZrotationAngle - self.old_LinkRotation.toEuler()[2]))
    ...
    def splitExpressionLink(self, expr, parent):
        ...
        if restFinal != '':
            restFinal = restFinal[1:3] if restFinal.startswith(
                '(') else restFinal[0:2]
    ...
    def initUI(self):
        ...
        self.parentList.addItem(App.Qt.translate("Assembly", 'Please select'))
        self.parentTable.append(self.rootAssembly)
        parentIcon = self.rootAssembly.ViewObject.Icon
        self.parentList.addItem(
            parentIcon, 'Parent Assembly', self.rootAssembly)
    ...
    def drawUI(self):
        ...
        self.leftLayout.addWidget(QtGui.QLabel(
            App.Qt.translate("Assembly", "Linked Part :")))
        ...
        self.leftLayout.addWidget(QtGui.QLabel(
            App.Qt.translate("Assembly", "Select LCS in Part :")))
        self.partLCSlist = QtGui.QListWidget(self.form)
        self.partLCSlist.setMinimumHeight(200)
        self.partLCSlist.setToolTip(App.Qt.translate(
            "Assembly", 'Select a coordinate system from the list'))
        self.leftLayout.addWidget(self.partLCSlist)

        # Assembly, Right side
        #
        # the document containing the linked object
        self.rightLayout.addWidget(QtGui.QLabel(
            App.Qt.translate("Assembly", "Parent Part :")))
        ...
        self.rightLayout.addWidget(QtGui.QLabel(
            App.Qt.translate("Assembly", "Select LCS in Parent :")))
        self.attLCSlist = QtGui.QListWidget(self.form)
        self.attLCSlist.setMinimumHeight(200)
        self.attLCSlist.setToolTip(App.Qt.translate(
            "Assembly", 'Select a coordinate system from the list'))
        ...
        self.XtranslSpinBoxLabel = self.XoffsetLayout.addWidget(
            QtGui.QLabel(App.Qt.translate("Assembly", "X Translation :")))
        ...
        self.XtranslSpinBox.setToolTip(App.Qt.translate(
            "Assembly", "Translation along X axis"))
        self.RotXButton = QtGui.QPushButton(
            App.Qt.translate("Assembly", 'Rotate X +90°'))
        self.RotXButton.setToolTip(App.Qt.translate(
            "Assembly", "Rotate 90 deg around X axis"))
        ...
        self.YtranslSpinBoxLabel = self.YoffsetLayout.addWidget(
            QtGui.QLabel(App.Qt.translate("Assembly", "Y Translation :")))
        ...
        self.YtranslSpinBox.setToolTip(
            App.Qt.translate("Assembly", "Translation along Y"))
        self.RotYButton = QtGui.QPushButton(
            App.Qt.translate("Assembly", 'Rotate Y +90°'))
        self.RotYButton.setToolTip(App.Qt.translate(
            "Assembly", "Rotate 90 deg around Y axis"))
        # add the QLDoubleSpinBox
        ...
        self.ZtranslSpinBoxLabel = self.ZoffsetLayout.addWidget(
            QtGui.QLabel(App.Qt.translate("Assembly", "Z Translation :")))
        ...
        self.ZtranslSpinBox.setToolTip(
            App.Qt.translate("Assembly", "Translation along Z:"))
        self.RotZButton = QtGui.QPushButton(
            App.Qt.translate("Assembly", 'Rotate Z +90°'))
        self.RotZButton.setToolTip(App.Qt.translate(
            "Assembly", "Rotate 90 deg around Z axis"))
        # add to the layout
