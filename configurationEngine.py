...
def createConfig(name, description):
    ...
    if not group:
        ...
        if assy:
            ...
            FCC.PrintWarnin(_atr("Asm4_Configurations",
                            'No assembly container here, quitting\n'))
            return
    # Create the document
...
class openConfigurationsUI():
    def __init__(self):
        ...
        self.form.setWindowTitle(
            _atr("Asm4_Configurations", 'Assembly Configurations'))
    ...
    def Restore(self):
        ...
        if len(selectedItems) != 1:
            Asm4.warningBox(_atr("Asm4_Configurations",
                            'Please select a configuration in the list'))
            return
    ...
    def onDelete(self):
        ...
        if len(selectedItems) == 1:
            ...
            if isAsm4Config(conf):
                confirm = Asm4.confirmBox(_atr(
                    "Asm4_Configurations", 'This will delete configuration "') + confName + '"?')
                if confirm:
                    App.ActiveDocument.removeObject(confName)
                else:
                    FCC.PrintMessage(_atr("Asm4_Configurations", 'Configuration "') +
                                     confName + _atr("Asm4_Configurations", '" not touched\n'))
            else:
                FCC.PrintMessage(_atr("Asm4_Configurations", 'Object "') + confName + _atr(
                    "Asm4_Configurations", '" is not a valid Assembly4 configuration, leaving untouched\n'))
    ...
    def drawUI(self):
        ...
        self.deleteButton = QtGui.QPushButton(
            _atr("Asm4_Configurations", 'Delete'))
        self.overwriteButton = QtGui.QPushButton(
            _atr("Asm4_Configurations", 'Overwrite'))
        # the button layout
...
class newConfigurationCmd:
    ...
    def drawUI(self):
        ...
        self.UI.setWindowTitle(
            _atr("Asm4_Configurations", 'Create a new assembly configuration'))
        ...
        # Configuration name
        self.mainLayout.addWidget(QtGui.QLabel(
            _atr("Asm4_Configurations", "Enter configuration name:")))
        self.configName = QtGui.QLineEdit()
        self.mainLayout.addWidget(self.configName)
        # Configuration description
        self.mainLayout.addWidget(QtGui.QLabel(
            _atr("Asm4_Configurations", "Description (optional):")))
        self.configDescription = QtGui.QTextEdit()
        self.configDescription.setMinimumHeight(100)
        self.mainLayout.addWidget(self.configDescription)
        # List of configurations
        self.mainLayout.addWidget(QtGui.QLabel(
            _atr("Asm4_Configurations", "Existing configurations:")))
        self.configList = QtGui.QListWidget()
        self.configList.setMinimumHeight(100)
        self.mainLayout.addWidget(self.configList)
        ...
        self.CancelButton = QtGui.QPushButton(
            _atr("Asm4_Configurations", 'Cancel'))
        # OK button
...
def SaveConfiguration(confName, description):
    FCC.PrintMessage(_atr("Asm4_Configurations",
                     'Saving configuration to "') + confName + '"\n')
    #conf = getConfig(confName, 'Configurations')
    conf = getConfig(confName)
    if conf and isAsm4Config(conf):
        confirm = Asm4.confirmBox(_atr(
            "Asm4_Configurations", 'This will overwrite existing configuration "') + confName + '"')
        if not confirm:
            FCC.PrintMessage(
                _atr("Asm4_Configurations", 'Cancel save of configuration "') + confName + '"\n')
            return
        else:
...
def restoreConfiguration(confName):
    FCC.PrintMessage(_atr("Asm4_Configurations",
                     'Restoring configuration "') + confName + '"\n')
...
def restoreObject(conf, obj):
    ...
    if row is None:
        if obj.isDerivedFrom('Part::Feature') or obj.isDerivedFrom('App::Link'):
            FCC.PrintMessage(_atr("Asm4_Configurations", 'No data for object "') + objName +
                             _atr("Asm4_Configurations", '" in configuration "') + conf.Name + '"\n')
        return
    ...
    try:
        ...
        FCC.PrintMessage(_atr("Asm4_Configurations", 'Unknown AssemblyType "')+asmType+_atr("Asm4_Configurations",
                         '" for object "') + objName + _atr("Asm4_Configurations", '" in configuration "') + conf.Name + '"\n')
