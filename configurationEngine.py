...
def createConfig(name, description):
    ...
    if not group:
        ...
        if assy:
            group = assy.newObject(
                'App::DocumentObjectGroup', 'Configurations')
        else:
            FCC.PrintWarnin(App.Qt.translate(
                "Assembly", 'No assembly container here, quitting\n'))
            return
    # Create the document
...
class applyConfigurationCmd:
    ...
    def GetResources(self):
        return {"MenuText": App.Qt.translate("Assembly", "Apply configuration"),
                "ToolTip": App.Qt.translate("Assembly", "Applies selected configuration\nConfigurations allow to set visibilities and offsets of parts"),
...
class openConfigurationsCmd:
    ...
    def GetResources(self):
        return {"MenuText": App.Qt.translate("Assembly", "Open configurations panel"),
                "ToolTip": App.Qt.translate("Assembly", "Configurations allow to set visibilities and offsets of parts"),
...
class openConfigurationsUI():
    ...
    def Restore(self):
        ...
        if len(selectedItems) != 1:
            Asm4.warningBox(App.Qt.translate(
                "Assembly", 'Please select a configuration in the list'))
            return
    ...
    def onDelete(self):
        ...
        if len(selectedItems) == 1:
            ...
            if isAsm4Config(conf):
                confirm = Asm4.confirmBox(App.Qt.translate(
                    "Assembly", 'This will delete configuration "') + confName + '"?')
                if confirm:
                    App.ActiveDocument.removeObject(confName)
                else:
                    FCC.PrintMessage(App.Qt.translate("Assembly", 'Configuration "') +
                                     confName + App.Qt.translate("Assembly", '" not touched\n'))
            else:
                FCC.PrintMessage(App.Qt.translate("Assembly", 'Object "') + confName + App.Qt.translate(
                    "Assembly", '" is not a valid Assembly4 configuration, leaving untouched\n'))
    ...
    def drawUI(self):
        ...
        self.newButton = QtGui.QPushButton(App.Qt.translate("Assembly", 'New'))
        self.deleteButton = QtGui.QPushButton(
            App.Qt.translate("Assembly", 'Delete'))
        self.overwriteButton = QtGui.QPushButton(
            App.Qt.translate("Assembly", 'Overwrite'))
        # the button layout
...
class newConfigurationCmd:
    ...
    def GetResources(self):
        return {"MenuText": App.Qt.translate("Assembly", "New configuration"),
                "ToolTip": App.Qt.translate("Assembly", "Create a new configuration of the assembly"),
                "Pixmap": os.path.join(Asm4.iconPath, 'Asm4_applyConfig.svg')
                }
    ...
    def onOK(self):
        ...
        if confName == '':
            Asm4.warningBox(App.Qt.translate(
                "Assembly", 'Please specify configuration name!'))
    ...
    def drawUI(self):
        ...
        self.UI.setWindowTitle(App.Qt.translate(
            "Assembly", 'Create a new assembly configuration'))
        ...
        # Configuration name
        self.mainLayout.addWidget(QtGui.QLabel(
            App.Qt.translate("Assembly", "Enter configuration name:")))
        self.configName = QtGui.QLineEdit()
        self.mainLayout.addWidget(self.configName)
        # Configuration description
        self.mainLayout.addWidget(QtGui.QLabel(
            App.Qt.translate("Assembly", "Description (optional):")))
        self.configDescription = QtGui.QTextEdit()
        self.configDescription.setMinimumHeight(100)
        self.mainLayout.addWidget(self.configDescription)
        # List of configurations
        self.mainLayout.addWidget(QtGui.QLabel(
            App.Qt.translate("Assembly", "Existing configurations:")))
        self.configList = QtGui.QListWidget()
        self.configList.setMinimumHeight(100)
        self.mainLayout.addWidget(self.configList)
...
def SaveConfiguration(confName, description):
    FCC.PrintMessage(App.Qt.translate(
        "Assembly", 'Saving configuration to "') + confName + '"\n')
    #conf = getConfig(confName, 'Configurations')
    conf = getConfig(confName)
    if conf and isAsm4Config(conf):
        confirm = Asm4.confirmBox(App.Qt.translate(
            "Assembly", 'This will overwrite existing configuration "') + confName + '"')
        if not confirm:
            FCC.PrintMessage(App.Qt.translate(
                "Assembly", 'Cancel save of configuration "') + confName + '"\n')
            return
        else:
...
def SaveObject(conf, obj):
    ...
    # if it's an Assembly4 object
        ...
        conf.set(OFFSET_ROT_PITCH_COL + row,
                 str(offset.Rotation.toEuler()[1]))
...
def restoreConfiguration(confName):
    FCC.PrintMessage(App.Qt.translate(
        "Assembly", 'Restoring configuration "') + confName + '"\n')
...
def restoreObject(conf, obj):
    ...
    if row is None:
        if obj.isDerivedFrom('Part::Feature') or obj.isDerivedFrom('App::Link'):
            FCC.PrintMessage(App.Qt.translate("Assembly", 'No data for object "') + objName +
                             App.Qt.translate("Assembly", '" in configuration "') + conf.Name + '"\n')
        return
    ...
    try:
        ...
        FCC.PrintMessage(App.Qt.translate("Assembly", 'Unknown AssemblyType "')+asmType+App.Qt.translate("Assembly",
                         '" for object "') + objName + App.Qt.translate("Assembly", '" in configuration "') + conf.Name + '"\n')
