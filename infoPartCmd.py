...
class infoPartCmd():
    ...
    def GetResources(self):
        tooltip = App.Qt.translate(
            "Assembly", "<p>Edit Part information</p><p>User-supplied information can be added to a part</p>")
        iconFile = os.path.join(Asm4.iconPath, 'Asm4_PartInfo.svg')
        return {"MenuText": App.Qt.translate("Assembly", "Edit Part Information"), "ToolTip": tooltip, "Pixmap": iconFile}
...
class infoPartUI():
    ...
    def __init__(self):
        ...
        self.form.setWindowTitle(App.Qt.translate(
            "Assembly", "Edit Part Information"))
    ...
    def reInit(self):
        ...
        Asm4.warningBox(App.Qt.translate(
            "Assembly", "The Part Info field has been cleared"))
        self.finish()
    ...
    def ShapeVolume(self, part, body):
        ...
        auto_info_fill = str(str(round(bbc.ZLength, 3)) + str(' mm x ') + str(
            round(bbc.YLength, 3)) + str(' mm x ') + str(round(bbc.XLength, 3)) + str(' mm'))
        try:
            self.TypeId
            setattr(part, auto_info_field, auto_info_fill)
        except AttributeError:
    ...
    def drawUI(self):
        ...
        self.confFields = QtGui.QPushButton(
            App.Qt.translate("Assembly", 'Configure fields'))
        self.confFields.setToolTip(App.Qt.translate("Assembly", 'Edit fields'))
        self.reinit = QtGui.QPushButton(
            App.Qt.translate("Assembly", 'Reset fields'))
        self.reinit.setToolTip(App.Qt.translate("Assembly", 'Reset fields'))
        self.autoFill = QtGui.QPushButton(
            App.Qt.translate("Assembly", 'Autofill data'))
        self.autoFill.setToolTip(
            App.Qt.translate("Assembly", 'Autofill fields'))
...
class infoPartConfUI():
        ...
        for prop in self.confTemplate:
            if self.confTemplate.get(prop).get('visible'):
                if self.infos[i].text() == '':
                    ...
                    '''
                    Asm4.warningBox(App.Qt.translate(
                        "Assembly", "Fields Name cannot be blank. You must disable or delete it"))
                    return
                i += 1
    ...
    def drawConfUI(self):
        ...
        self.addnew = QtGui.QPushButton(App.Qt.translate("Assembly", 'Add'))
        self.gridLayoutButtons.addWidget(self.addnew, 0, 2)
        self.suppBut = QtGui.QPushButton(
        ...
        if self.updateAutoFieldlist() != None:
            updateLab = QtGui.QLabel(App.Qt.translate(
                "Assembly", 'Update automatic input field'))
            ...
            self.upBut = QtGui.QPushButton(
                App.Qt.translate("Assembly", 'Update'))
