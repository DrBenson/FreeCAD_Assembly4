...
class animationExporter():
    ...
    def writeVideo(self, filename):
        ...
        for fcc in fourccs:
            ...
            if os.path.getsize(filename) > 0:
                ...
                break
        if not exported:
            App.Console.PrintError(_atr("Asm4_Animate", "Export failed for \"") + filename + _atr(
                "Asm4_Animate", "\". Using another container type can help.\n"))
    ...
    def createProgressDlg(self):
        pDlg = QtGui.QProgressDialog(_atr("Asm4_Animate", "Capturing and Exporting..."), _atr(
            "Asm4_Animate", "Cancel"), 0, 100)
...
class exportDialog(QtGui.QDialog):
    def __init__(self, owner: animationExporter, parentWidget=None):
        ...
        self.gpbPreview = QtGui.QGroupBox(
            _atr("Asm4_Animate", "Preview"), self)
        self.lblPreview = QtGui.QLabel(
            _atr("Asm4_Animate", 'TheImageLabel'), self.gpbPreview)
        ...
        self.prevHL.addWidget(QtGui.QLabel(
            _atr("Asm4_Animate", "Preview Scale:")))
        ...
        self.outputFileSel = fileSelectorWidget(
            _atr("Asm4_Animate", "save"), self.gpbOutput)
        tt = _atr(
            "Asm4_Animate", 'Choose a file\nSupported extensions are *.gif *.mp4 *.avi *.mov *.mkv')
        ...
        self.bgImgFileSel = fileSelectorWidget(
            _atr("Asm4_Animate", "read"), self.gpbBG)
        tt = _atr(
            "Asm4_Animate", 'Choose an image file\nSupported extensions are *.png *.jpg *.jpeg *.gif')
        ...
        self.logoFileSel = fileSelectorWidget(
            _atr("Asm4_Animate", "read"), self.gpbLogo)
    ...
    # # # buttons incl. lower layout # # #
        ...
        self.pbRefreshPreview = QtGui.QPushButton(
            _atr("Asm4_Animate", "Refresh Preview"))
        self.pbCreateAndSave = QtGui.QPushButton(
            _atr("Asm4_Animate", "Create and Save"))
...
class fileSelectorWidget(QtGui.QWidget):
    ...
    def __init__(self, type="read", parent=None):
        ...
        self.pbSelectFile = QtGui.QPushButton(
            _atr("Asm4_Animate", "Select"), parent)
        ...
        self.filter = _atr("Asm4_Animate", "Image Files (*.png *.jpg *.jpeg *.gif)") if self.type == "read" else _atr(
            "Asm4_Animate", "Supported Files (*.mp4 *.avi *.mov *.mkv *.gif *.png)")
