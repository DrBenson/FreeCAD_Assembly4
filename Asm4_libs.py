    ...
    if variables:
        if variables.TypeId == 'App::FeaturePython':
            ...
            FCC.PrintWarning(App.Qt.translate(
                "Assembly", 'This Part contains an incompatible \"Variables\" object, '))
            FCC.PrintWarning(App.Qt.translate(
                "Assembly", 'this could lead to unexpected results\n'))
    # there is none, so we create it
    else:
        variables = App.ActiveDocument.addObject(
            'App::FeaturePython', 'Variables')
        variables.ViewObject.Proxy = setCustomIcon(
            object, 'Asm4_Variables.svg')
...
def getSelectionTree(index=0):
    ...
    if len(Gui.Selection.getSelection()) > index:
        ...
        if len(Gui.Selection.getSelectionEx("", 0)[0].SubElementNames) >= index:
            ...
            selList = Gui.Selection.getSelectionEx(
                "", 0)[0].SubElementNames[index]
    ...
    for Selection in CompleteSelection:
        ...
        if hasattr(Selection, 'getSubObjects'):
            ...
            for SubObjName in SubObjNames:
                SubObjects.append(
                    App.ActiveDocument.getObject(SubObjName[0:-1]))
    ...
    # DEPRECATED, to be removed
        ...
        if obj.AssemblyType == 'Asm4EE' or obj.AssemblyType == '':
            FCC.PrintMessage(App.Qt.translate(
                "Assembly", 'Found legacy AssemblyType property, adding new empty SolverId property\n'))
...
def warningBox(text):
    ...
    msgBox.setWindowTitle(App.Qt.translate("Assembly", 'FreeCAD Warning'))
...
def confirmBox(text):
    ...
    msgBox.setText(text)
    msgBox.setInformativeText(App.Qt.translate(
        "Assembly", 'Are you sure you want to proceed ?'))
...
class dropDownCmd:
    def __init__(self, cmdlist, menu, tooltip=None, icon=None):
        ...
        self.menu = App.Qt.translate("Assembly", menu)
        if tooltip is None:
            self.tooltip = menu
        else:
            self.tooltip = App.Qt.translate("Assembly", tooltip)
