...
class newPart:
    ...
    def __init__(self, partName):
        ...
        if self.partName == 'Part':
            ...
            self.menutext = App.Qt.translate("Assembly", "New Part")
            self.tooltip = App.Qt.translate("Assembly", "Create a new Part")
            self.icon = os.path.join(Asm4.iconPath, 'Asm4_Part.svg')
        elif self.partName == 'Body':
            self.partType = 'PartDesign::Body'
            self.menutext = App.Qt.translate("Assembly", "New Body")
            self.tooltip = App.Qt.translate("Assembly", "Create a new Body")
            self.icon = os.path.join(Asm4.iconPath, 'Asm4_Body.svg')
        elif self.partName == 'Group':
            self.partType = 'App::DocumentObjectGroup'
            self.menutext = App.Qt.translate("Assembly", "New Group")
            self.tooltip = App.Qt.translate("Assembly", "Create a new Group")
    ...
    def Activated(self):
        ...
        text, ok = QtGui.QInputDialog.getText(None, self.tooltip, App.Qt.translate(
            "Assembly", 'Enter new ')+self.partName+App.Qt.translate("Assembly", ' name :')+' '*30, text=instanceName)
        if ok and text:
            # create Part
            ...
            if self.partType in Asm4.containerTypes:
                ...
                lcs0 = newPart.newObject(
                    'PartDesign::CoordinateSystem', 'LCS_0')
