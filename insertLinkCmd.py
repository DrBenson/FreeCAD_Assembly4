...
class insertLink():
    ...
    def GetResources(self):
        tooltip = App.Qt.translate("Assembly", "<p>Insert a Part into the assembly. " +
        ...
        return {"MenuText": App.Qt.translate("Assembly", "Insert Part"),
                "ToolTip": tooltip,
    ...
    def Activated(self):
            ...
            if selObj.isDerivedFrom('App::Link') and selObj.LinkedObject is None:
                ...
                if parent.TypeId == 'App::Part' and parent.getParentGeoFeatureGroup() is None:
                    ...
                    self.UI.setWindowTitle(App.Qt.translate(
                        "Assembly", 'Re-link broken link'))
                    self.insertButton.setText(
                        App.Qt.translate("Assembly", 'Replace'))
        ...
        if self.rootAssembly is None:
            Asm4.warningBox(App.Qt.translate(
                "Assembly", 'Please create an Assembly'))
            return
    ...
    def openFile(self):
        ...
        dialog = QtGui.QFileDialog(QtGui.QApplication.activeWindow(),
                                   App.Qt.translate("Assembly", "Select FreeCAD document to import part from"))
        ...
        '''
        dialog.setNameFilter(App.Qt.translate(
            "Assembly", "Supported Formats *.FCStd *.fcstd (*.FCStd *.fcstd);;All files (*.*)"))
        if dialog.exec_():
    ...
    def onCreateLink(self):
        ...
        # only create link if there is a Part object and a name
            ...
            if App.ActiveDocument.FileName != '' or App.ActiveDocument == selectedPart.Document:
                ...
                Asm4.warningBox(App.Qt.translate(
                    "Assembly", 'The current document must be saved before inserting an external part'))
                return
    ...
    def drawUI(self):
        ...
        self.UI.setWindowTitle(App.Qt.translate("Assembly", 'Insert a Part'))
        ...
        self.cancelButton = QtGui.QPushButton(
            App.Qt.translate("Assembly", 'Cancel'), self.UI)
        # Cancel button
        self.openFileButton = QtGui.QPushButton(
            App.Qt.translate("Assembly", 'Open file'), self.UI)
        # Insert Link button
        self.insertButton = QtGui.QPushButton(
            App.Qt.translate("Assembly", 'Insert'), self.UI)
        self.insertButton.setDefault(True)

        # Place the widgets with layouts
        self.mainLayout = QtGui.QVBoxLayout(self.UI)
        self.mainLayout.addWidget(QtGui.QLabel(
            App.Qt.translate("Assembly", "Filter :")))
        self.mainLayout.addWidget(self.filterPartList)
        self.mainLayout.addWidget(QtGui.QLabel(
            App.Qt.translate("Assembly", "Select Part to be inserted :")))
        self.mainLayout.addWidget(self.partList)
        self.mainLayout.addWidget(QtGui.QLabel(
            App.Qt.translate("Assembly", "Name for the link :")))
