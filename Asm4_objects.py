...
class VariantLink(object):
    def __init__(self):
        FCC.PrintMessage(App.Qt.translate("Assembly", 'Initialising ...')+'\n')
        self.Object = None
    ...
    def makeVarLink(self, obj):
        ...
        if i < 100:
            ...
            FCC.PrintWarning(App.Qt.translate("Assembly", '100 temporary variant documents are already in use, not creating a new one.')+'\n')
        return
    ...
    def fillVarProperties(self, obj):
        ...
        if variables is None:
            FCC.PrintVarning(
                App.Qt.translate("Assembly", 'No \"Variables\" container in source object')+'\n')
        else:
    ...
    def onSettingDocument(self, obj):
        FCC.PrintMessage(
            App.Qt.translate("Assembly", 'Triggered onSettingDocument() in VariantLink')+'\n')
        obj.LinkedObject = obj.SourceObject
        return
    ...
    def onLostLinkToObject(self, obj):
        FCC.PrintMessage(
            App.Qt.translate("Assembly", 'Triggered onLostLinkToObject() in VariantLink')+'\n')
        obj.LinkedObject = obj.SourceObject
        return
    ...
    def setupObject(self, obj):
        FCC.PrintMessage(
            App.Qt.translate("Assembly", 'Triggered by setupObject() in VariantLink')+'\n')
        obj.LinkedObject = obj.SourceObject
...
class LinkArray(object):
    ...
    def attach(self, obj):
        ...
        obj.addProperty("App::PropertyLink",   "SourceObject",
                        "Array", App.Qt.translate("Assembly", 'The object to array'))
        ...
        obj.addProperty("App::PropertyInteger", "Count",        "Array",
                        App.Qt.translate("Assembly", 'Total number of elements in the array'))
        obj.Count = 1
        # install the actual extension
        obj.addExtension('App::LinkExtensionPython')
        # initiate the extension
...
class ExpressionArray(LinkArray):
    ...
    def attach(self, obj):
        ...
        obj.addProperty('App::PropertyPlacement',   'Placer',           'Array',
                        App.Qt.translate("Assembly",'Calculates element placements in relation to the Axis.\n'
        ...
        obj.addProperty('App::PropertyInteger',
                        'Index',            'Array', '')
        obj.addProperty('App::PropertyLinkSub',     'Axis',             'Array',
                        App.Qt.translate("Assembly",'The axis, direction or plane the Placer relates to'))
        obj.addProperty('App::PropertyPlacement',
                        'AxisPlacement',     'Array', '')
        obj.addProperty('App::PropertyFloat',
                        'Scaler',            'Array', '')
    ...
    def execute(self, obj):
        ...
        if not obj.SourceObject:
            self.raiseError(obj, App.Qt.translate("Assembly", "Missing Source Object"))
        sObj = obj.SourceObject
        # we only deal with objects that are in a parent container because we'll put the array there
        parent = sObj.getParentGeoFeatureGroup()
        if not parent:
            self.raiseError(obj, App.Qt.translate("Assembly", "Source Object must reside inside a Part"))
        # find placement of axis
        if obj.Axis:
            if parent != obj.Axis[0].getParentGeoFeatureGroup():
                self.raiseError(
                    obj, App.Qt.translate("Assembly", 'Source Object and Axis must have the same parent Part'))
            obj.AxisPlacement = findAxisPlacement(*obj.Axis)
            if obj.AxisPlacement is None:
                self.raiseError(
                    obj, App.Qt.translate("Assembly", 'The type of the selected axis is not supported'))
        else:
                ...
                if _findParam(n, exDict[edge]):
                    if edge not in resolved:
                        if edge in unresolved:
                            raise RuntimeError(
                                App.Qt.translate("Assembly", 'Circular reference detected: {} -> {}').format(n, edge))
