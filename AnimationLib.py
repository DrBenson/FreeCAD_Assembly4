...
class animationProvider:
    ...
    def nextFrame(self, resetAnimation) -> bool:
        raise NotImplementedError(App.Qt.translate(
            "Assembly", "animationProvider.nextFrame not implemented."))
    ...
    class Error(Exception):
        ...
        def __init__(self, shortMsg: str, detailMsg: str):
            self.shortMsg = QT_TRANSLATE_NOOP("Assembly", shortMsg)
            self.detailMsg = QT_TRANSLATE_NOOP("Assembly", detailMsg)
...
class animateVariable(animationProvider):
    ...
    class variableInvalidError(animationProvider.Error):
        ...
        def __init__(self, varName):
            shortMsg = QT_TRANSLATE_NOOP("Assembly", 'Variable name invalid')
            detailMsg = QT_TRANSLATE_NOOP("Assembly", 'The selected variable name "') + varName + QT_TRANSLATE_NOOP(
                "Assembly", '" is not valid. ') + QT_TRANSLATE_NOOP("Assembly", 'Please select an existing variable.')
    ...
    def __init__(self):
        ...
        # True flags when the animation is "in reverse" for the pendulum mode.
        self.reverseAnimation = False
        # True Forces GUI to update on every step of the animation.
        self.ForceGUIUpdate = False
    ...
    def GetResources(self):
        return {"MenuText": App.Qt.translate("Assembly", "Animate Assembly"),
                "ToolTip": App.Qt.translate("Assembly", "Animate Assembly"),
    ...
    def updateDocList(self):
        docDocs = [App.Qt.translate("Assembly", '- Select Document -')]
    ...
    def updateVarList(self):
        docVars = [App.Qt.translate(
            "Assembly", '- Select Variable (only float) -')]
    ...
    def update(self, req):
        ...
        # RUNNING STATE
            ...
            stop |= endOfCycle and not (
                self.Pendulum.isChecked() or self.Loop.isChecked())
        ...
        # SANITY CHECK
            ...
            print(App.Qt.translate("Assembly", "Unknown State/Transition"))
    ...
    def onLoop(self):
        aniHints = animationHints.get(
            self.Variables, self.varList.currentText())
    ...
    def updateSlider(self):
        ...
        rangeShort = (self.slider.rightValue() < endVal) if (
            beginVal < endVal) else (self.slider.rightValue() > endVal)
        if rangeShort:
    ...
    def onClose(self):
        self.onStop()
        animationHints.cleanUp(self.Variables)
        self.MDIArea.subWindowActivated[QtGui.QMdiSubWindow].disconnect(
            self.onDocChanged)
        self.UI.close()
    ...
    def onSave(self):
        ...
        try:
            ...
            Asm4.warningBox(App.Qt.translate(
                "Assembly", 'The Python module \"OpenCV\" is not installed'))
    ...
    def drawUI(self):
        ...
        self.formLayout.addRow(QtGui.QLabel(
            App.Qt.translate("Assembly", 'Document')), self.docList)
        # select Variable
        self.varList = updatingComboBox()
        self.formLayout.addRow(QtGui.QLabel(
            App.Qt.translate("Assembly", 'Variable')), self.varList)
        # Range Minimum
        self.beginValue = QtGui.QDoubleSpinBox()
        self.beginValue.setRange(numpy.finfo(
            float).min, numpy.finfo(float).max)
        self.beginValue.setKeyboardTracking(False)
        self.formLayout.addRow(QtGui.QLabel(App.Qt.translate(
            "Assembly", 'Range Begin')), self.beginValue)
        # Maximum
        ...
        self.formLayout.addRow(QtGui.QLabel(
            App.Qt.translate("Assembly", 'Range End')), self.endValue)
        # Step
        ...
        self.formLayout.addRow(QtGui.QLabel(
            App.Qt.translate("Assembly", 'Step Size')), self.stepValue)
        ...
        self.formLayout.addRow(QtGui.QLabel(App.Qt.translate(
            "Assembly", 'Sleep (s)')), self.sleepValue)
        # apply the layout
        ...
        self.variableValue = QtGui.QLabel(
            App.Qt.translate("Assembly", 'Variable'))
        self.curVarLayout.addWidget(QtGui.QLabel(
            App.Qt.translate("Assembly", 'Current Value:')))
        ...
        self.sliderLeftValue = QtGui.QLabel(
            App.Qt.translate("Assembly", 'Begin'))
        self.sliderRightValue = QtGui.QLabel(
            App.Qt.translate("Assembly", 'End'))
        tt = App.Qt.translate("Assembly", "The last reachable variable value with the given stepping. " +
        ...
        self.ForceRender.setToolTip(App.Qt.translate(
            "Assembly", "Force GUI to update on every step."))
        self.ForceRender.setText(App.Qt.translate(
            "Assembly", "Force-render every step"))
        self.ForceRender.setChecked(False)

        self.Loop = QtGui.QCheckBox()
        self.Loop.setLayoutDirection(QtCore.Qt.RightToLeft)
        self.Loop.setToolTip(App.Qt.translate("Assembly", "Infinite Loop"))
        self.Loop.setText(App.Qt.translate("Assembly", "Loop"))
        self.Loop.setChecked(False)

        self.Pendulum = QtGui.QCheckBox()
        self.Pendulum.setLayoutDirection(QtCore.Qt.RightToLeft)
        self.Pendulum.setToolTip(App.Qt.translate(
            "Assembly", "Back-and-forth pendulum"))
        self.Pendulum.setText(App.Qt.translate("Assembly", "Pendulum"))
        self.Pendulum.setChecked(False)
        ...
        self.CloseButton = QtGui.QPushButton(
            App.Qt.translate("Assembly", 'Close'))
        self.CloseButton.setToolTip(App.Qt.translate("Assembly", "Exit"))
        self.buttonLayout.addWidget(self.CloseButton)
        self.buttonLayout.addStretch()
        # Export button
        self.SaveButton = QtGui.QPushButton(
            App.Qt.translate("Assembly", 'Save'))
        self.SaveButton.setToolTip(App.Qt.translate(
            "Assembly", "Save this sequence as video"))
        self.buttonLayout.addWidget(self.SaveButton)
        self.buttonLayout.addStretch()
        # Stop button
        self.StopButton = QtGui.QPushButton(
            App.Qt.translate("Assembly", 'Stop'))
        self.buttonLayout.addWidget(self.StopButton)
        self.buttonLayout.addStretch()
        self.StopButton.setEnabled(False)
        # Run button
        self.RunButton = QtGui.QPushButton(App.Qt.translate("Assembly", 'Run'))
        tt = App.Qt.translate("Assembly", "Run this sequence in the 3D window" +
        ...
        self.DummyButton = QtGui.QPushButton(
            App.Qt.translate("Assembly", 'Dummy'))
...
class animationHints():
    ...
    def __getHintList__(variables):
        ...
        if "AnimationHintList" not in variables.PropertiesList:
            variables.addProperty("App::PropertyPythonObject", "AnimationHintList", "AnimationHints", App.Qt.translate(
                "Assembly", "The hintfield for the animation dialog")).AnimationHintList = {}
